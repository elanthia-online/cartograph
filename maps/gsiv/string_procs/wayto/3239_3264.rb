r = dothistimeout 'open rolaren gate', 10, /^You open|^That is already open|^It appears to be locked/
 if r =~ /^You open|^That is already open/
 move 'go rolaren gate'
 elsif r =~ /^It appears to be locked/
 spell = Spell['sigil of resolve']
 bonus = spell.known? ? Society.rank/2 : 0
 ...
 1.times { unless Spell[num].affordable?
 echo 'waiting for mana...'
 ...
 redo if r =~ /^\[Spell Hindrance|^The gate vibrates slightly but nothing else happens|^A translucent force slams into the gate, but it remains unaffected/ }
 move 'go rolaren gate'
 else
