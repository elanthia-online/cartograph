unless (move 'go door')
 ...
 unless push_result.nil? or push_result == 'You grasp the top tine and try to turn it, but it won\'t even budge.'
 unless mana < 100
 castables = [ 110, 120, 140, 210, 216, 230, 420, 425, 430, 520, 540, 620, 625, 650, 1040, 1110, 1115, 1601, 1602, 1603, 1604, 1607, 1613, 1614, 1615 ].sort { |a,b| a.to_s[-2..-1] <=> b.to_s[-2..-1] }.reverse.collect { |n| Spell[n] }
 ...
 mana_needed = 100
 while (mana_needed > 0) and ((Spell[515].active? and (spell = mana5spells.find { |s| s.known? })) or (spell = castables.find { |s| s.mana_cost <= mana_needed }) or (spell = castables.reverse.find { |s| s.mana_cost >= mana_needed }))
 loop { cast_result = spell.cast('crown')
 break unless cast_result =~ /^\[Spell Hindrance for/ }
 mana_needed -= spell.mana_cost
 sleep 0.1
 end
 end
 fput 'release' if Spell[515].active? and (checkprep != 'None')
 fput 'touch crown'
