...
if UserVars.mapdb_use_day_pass == true || UserVars.mapdb_use_day_pass =~ /^yes$/i
   ...
   if $mapdb_day_passes.any? { |id,h| h[:towns].include?("Ta'Illistim") and h[:towns].include?("Ta'Vaalor") and h[:expires] > (Time.now + 10) }
      0.8
   elsif UserVars.mapdb_buy_day_pass == true || UserVars.mapdb_buy_day_pass =~ /^yes$|\bill,val\b/i
      8.8
   else
      nil
   end
