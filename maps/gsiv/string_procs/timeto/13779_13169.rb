...
pass_id = $mapdb_day_passes.keys.find { |id| $mapdb_day_passes[id][:towns].include?("Ta'Illistim") and $mapdb_day_passes[id][:towns].include?("Ta'Vaalor") and $mapdb_day_passes[id][:expires] > (Time.now + 10) }
if pass_id
   dothistimeout "get ##{pass_id}", 10, get_regex
elsif UserVars.mapdb_buy_day_pass =~ /^yes$|\bill,val\b/i
   move 'go corridor'
   fput 'unhide' if hidden? or invisible?
   dothistimeout "ask halfling for Ta'Illistim", 10, /says to you/
   result = dothistimeout "ask halfling for Ta'Illistim", 10, /don't have enough|quickly hands you/
   if (result =~ /don't have enough/) and $go2_get_silvers
      ...
      fput 'unhide' if hidden? or invisible?
      result = dothistimeout "ask halfling for Ta'Illistim", 10, /says to you|don't have enough|quickly hands you/
